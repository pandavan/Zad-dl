...
# Copying and validation of url from clipbord using xsel
	...
	if [[ $clpy =~ $regex ]]
		...
		clpy="http://"
			...
			OUTPUT="$(yad --width="500" --title="Zad-dl" --text="Download audio and video files" --form --field="URL" --field="Directory:MDIR" --field="Proxychains:CHK" "$clpy" "$HOME/Downloads/" FALSE --button="Fetch URL:2" --button="gtk-ok:0" --button="gtk-cancel:1")"
...
if [ ! -x "$(command -v youtube-dl)" ] ||  [ ! -x "$(command -v xsel)" ] || [ ! -x "$(command -v proxychains)" ] \
	   ...
	   || [[ ! -d $HOME/.zad-dl ]] || [[ ! -f $HOME/.zad-dl/gtao4rj.png ]] \
	   || [[ ! -f $HOME/.zad-dl/Zad-dl.sh ]]; then
